import { Logo } from "@/components/ui/Logo";

export const Footer = () => {
  return (
    <footer className="border-t-gray-100 bg-gray-50 px-20 py-8 border-t border-solid max-sm:px-5 max-sm:py-8">
      <div className="max-w-screen-xl mx-auto my-0 max-md:px-10 max-md:py-0 max-sm:px-5 max-sm:py-0">
        <div className="mb-10">
          <Logo />
          <p className="text-sm text-gray-600 mt-4">
            Simple and secure time tracking for modern businesses.
          </p>
        </div>
        <div className="flex gap-8 mb-[35px] max-sm:flex-col max-sm:gap-6">
          <div className="flex-1">
            <h3 className="font-semibold text-base text-black mb-5">
              Quick Links
            </h3>
            <div className="flex flex-col gap-2">
              <a href="#" className="text-sm text-gray-600 no-underline">
                Features
              </a>
              <a href="#" className="text-sm text-gray-600 no-underline">
                Pricing
              </a>
              <a href="#" className="text-sm text-gray-600 no-underline">
                Support
              </a>
            </div>
          </div>
          <div className="flex-1">
            <h3 className="font-semibold text-base text-black mb-5">Company</h3>
            <div className="flex flex-col gap-2">
              <a href="#" className="text-sm text-gray-600 no-underline">
                About
              </a>
              <a href="#" className="text-sm text-gray-600 no-underline">
                Contact
              </a>
              <a href="#" className="text-sm text-gray-600 no-underline">
                Privacy
              </a>
            </div>
          </div>
          <div className="flex-1">
            <h3 className="font-semibold text-base text-black mb-5">Connect</h3>
            <div className="flex gap-4">
              {/* Social Icons */}
              <a href="#" className="text-gray-600">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M14.3553 4.741C14.3655 4.88313 14.3655 5.02529 14.3655 5.16741C14.3655 9.50241 11.066 14.4973 5.03553 14.4973C3.17766 14.4973 1.45178 13.9593 0 13.0253C0.263969 13.0557 0.51775 13.0659 0.791875 13.0659C2.32484 13.0659 3.73603 12.5481 4.86294 11.6649C3.42131 11.6344 2.21319 10.6903 1.79694 9.39075C2 9.42119 2.20303 9.4415 2.41625 9.4415C2.71066 9.4415 3.00509 9.40088 3.27919 9.32985C1.77666 9.02525 0.649719 7.70547 0.649719 6.11157V6.07097C1.08625 6.31463 1.59391 6.46691 2.13194 6.48719C1.24869 5.89835 0.670031 4.89329 0.670031 3.75622C0.670031 3.1471 0.832438 2.58872 1.11672 2.10141C2.73094 4.09125 5.15734 5.39072 7.87813 5.53288C7.82738 5.28922 7.79691 5.03544 7.79691 4.78163C7.79691 2.9745 9.25884 1.50244 11.0761 1.50244C12.0203 1.50244 12.873 1.89838 13.472 2.53797C14.2131 2.39585 14.9238 2.12172 15.5533 1.7461C15.3096 2.50754 14.7918 3.14713 14.1116 3.55319C14.7715 3.48216 15.4111 3.29938 15.9999 3.0456C15.5533 3.69532 14.9949 4.27397 14.3553 4.741Z"
                    fill="currentColor"
                  />
                </svg>
              </a>
              <a href="#" className="text-gray-600">
                <svg
                  width="14"
                  height="16"
                  viewBox="0 0 14 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M13 1H0.996875C0.446875 1 0 1.45313 0 2.00938V13.9906C0 14.5469 0.446875 15 0.996875 15H13C13.55 15 14 14.5469 14 13.9906V2.00938C14 1.45313 13.55 1 13 1ZM4.23125 13H2.15625V6.31875H4.23438V13H4.23125ZM3.19375 5.40625C2.52812 5.40625 1.99063 4.86562 1.99063 4.20312C1.99063 3.54063 2.52812 3 3.19375 3C3.85625 3 4.39687 3.54063 4.39687 4.20312C4.39687 4.86875 3.85938 5.40625 3.19375 5.40625ZM12.0094 13H9.93437V9.75C9.93437 8.975 9.91875 7.97813 8.85625 7.97813C7.775 7.97813 7.60938 8.82188 7.60938 9.69375V13H5.53438V6.31875H7.525V7.23125H7.55312C7.83125 6.70625 8.50938 6.15312 9.51875 6.15312C11.6187 6.15312 12.0094 7.5375 12.0094 9.3375V13Z"
                    fill="currentColor"
                  />
                </svg>
              </a>
              <a href="#" className="text-gray-600">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.75 8C15.75 3.71875 12.2812 0.25 8 0.25C3.71875 0.25 0.25 3.71875 0.25 8C0.25 11.8681 3.08406 15.0744 6.78906 15.6562V10.2403H4.82031V8H6.78906V6.2925C6.78906 4.35031 7.94531 3.2775 9.71625 3.2775C10.5644 3.2775 11.4513 3.42875 11.4513 3.42875V5.335H10.4738C9.51125 5.335 9.21094 5.9325 9.21094 6.54531V8H11.3603L11.0166 10.2403H9.21094V15.6562C12.9159 15.0744 15.75 11.8681 15.75 8Z"
                    fill="currentColor"
                  />
                </svg>
              </a>
              <a href="#" className="text-gray-600">
                <svg
                  width="14"
                  height="16"
                  viewBox="0 0 14 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M7.00303 4.40635C5.01553 4.40635 3.4124 6.00947 3.4124 7.99697C3.4124 9.98447 5.01553 11.5876 7.00303 11.5876C8.99053 11.5876 10.5937 9.98447 10.5937 7.99697C10.5937 6.00947 8.99053 4.40635 7.00303 4.40635ZM7.00303 10.3313C5.71865 10.3313 4.66865 9.28447 4.66865 7.99697C4.66865 6.70947 5.71553 5.6626 7.00303 5.6626C8.29053 5.6626 9.3374 6.70947 9.3374 7.99697C9.3374 9.28447 8.2874 10.3313 7.00303 10.3313ZM11.578 4.25947C11.578 4.7251 11.203 5.09697 10.7405 5.09697C10.2749 5.09697 9.90303 4.72197 9.90303 4.25947C9.90303 3.79697 10.278 3.42197 10.7405 3.42197C11.203 3.42197 11.578 3.79697 11.578 4.25947ZM13.9562 5.10947C13.903 3.9876 13.6468 2.99385 12.8249 2.1751C12.0062 1.35635 11.0124 1.1001 9.89053 1.04385C8.73428 0.978223 5.26865 0.978223 4.1124 1.04385C2.99365 1.09697 1.9999 1.35322 1.17803 2.17197C0.356152 2.99072 0.103027 3.98447 0.0467773 5.10635C-0.0188477 6.2626 -0.0188477 9.72822 0.0467773 10.8845C0.0999023 12.0063 0.356152 13.0001 1.17803 13.8188C1.9999 14.6376 2.99053 14.8938 4.1124 14.9501C5.26865 15.0157 8.73428 15.0157 9.89053 14.9501C11.0124 14.897 12.0062 14.6407 12.8249 13.8188C13.6437 13.0001 13.8999 12.0063 13.9562 10.8845C14.0218 9.72822 14.0218 6.26572 13.9562 5.10947ZM12.4624 12.1251C12.2187 12.7376 11.7468 13.2095 11.1312 13.4563C10.2093 13.822 8.02178 13.7376 7.00303 13.7376C5.98428 13.7376 3.79365 13.8188 2.8749 13.4563C2.2624 13.2126 1.79053 12.7407 1.54365 12.1251C1.17803 11.2032 1.2624 9.01572 1.2624 7.99697C1.2624 6.97822 1.18115 4.7876 1.54365 3.86885C1.7874 3.25635 2.25928 2.78447 2.8749 2.5376C3.79678 2.17197 5.98428 2.25635 7.00303 2.25635C8.02178 2.25635 10.2124 2.1751 11.1312 2.5376C11.7437 2.78135 12.2155 3.25322 12.4624 3.86885C12.828 4.79072 12.7437 6.97822 12.7437 7.99697C12.7437 9.01572 12.828 11.2063 12.4624 12.1251Z"
                    fill="currentColor"
                  />
                </svg>
              </a>
            </div>
          </div>
        </div>
      </div>
      <div className="text-sm text-gray-600 pt-[35px] border-t border-solid">
        © 2025 CICO Timeclock. All rights reserved.
      </div>
    </footer>
  );
};
